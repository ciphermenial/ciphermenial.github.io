...
> The previous guide no longer worked due to changes to XMR. It now treats the old tcp connections as legacy and now prioritises websockets.
{: .prompt-info }
...
This guide is now updated for installing [Xibo CMS 4.4.4](https://xibosignage.com/blog/june-2026-releases).
...
```bash
sudo tar -xzf xibo-cms-4.4.4.tar.gz --strip-components=1
sudo rm xibo-cms-4.4.4.tar.gz
```
...
```bash
sudo chown www-data: /srv/xibo-cms/vendor/bin/config.json
```
